...
int main(int argc, char *argv[]) {
  ...
  while (1) {
    ...
    conn_id =
        accept(master_socket, (struct sockaddr *)&client, (socklen_t *)&len);
    ...
    if (conn_id == -1) {
      ...
      if (connection >= CONCURRENT_CONNECTION) {
        ...
        printf("[WARNING] CONNECTION LIMITE REACHED");
        ...
        if (pthread_create(&thread_id, &attr, connection_handler,
                        int(conn_id)) == -1) {
          //  std::cout << "[WARNING][THREAD] " << strerror(errno) << "\n";
          printf("[WARNING][THREAD] 132");
...
printf("[TIME] PROCESS COMPLETE IN ");
            // << std::setprecision(5);
